...
csihandle(void)
{
    ...
    switch (csiescseq.arg[0]) {
    case 0: /* below */
        tclearregion(term.c.x, term.c.y, term.col-1, term.c.y, 1); // Add `1` as the 5th argument
        if (term.c.y < term.row-1) {
            tclearregion(0, term.c.y+1, term.col-1, term.row-1, 1); // Add `1` as the 5th argument
        }
        break;
    case 1: /* above */
        if (term.c.y > 0)
            tclearregion(0, 0, term.col-1, term.c.y-1, 1); // Add `1` as the 5th argument
        tclearregion(0, term.c.y, term.c.x, term.c.y, 1); // Add `1` as the 5th argument
        break;
    case 2: /* all */
        if (IS_SET(MODE_ALTSCREEN)) {
            tclearregion(0, 0, term.col-1, term.row-1, 1); // Add `1` as the 5th argument
            break;
        }
        /* vte does this:
